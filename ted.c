/*** includes ***/
#define _GNU_SOURCE //Avoid getline warning
#define _BSD_SOURCE 
...
int editorReadKey() { 
    ...
    if(c == '\x1b') {
        ...
        //char a[10];
        //int alen = snprintf(a, sizeof(a), "%c%c", seq[0],seq[1]);
        //write(STDOUT_FILENO, a, alen);
        //editorReadKey();
        if(seq[0] == '[') {
            if(seq[1] >= '0' && seq[1] < '9') {
...
void editorDelRow(int at) {
    ...
    for (int j = at; at < E.numrows - 1; j++) E.row[j].idx--;
    E.numrows--;
    E.dirty++;
}
        ...
        case CTRL_KEY('q'): 
            if(E.dirty && quit_times > 0) {
                editorSetStatusMessage("WARNING!!! File has unsaved changes."
                        "Presso Ctrl-Q %d more times to quit.", quit_times);
                quit_times--;
                return;
            }
            write(STDOUT_FILENO, "\x1b[2J", 4); 
